                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div class="border p-4 rounded-lg shadow-md">
                            ...
                            <div class="mt-4 flex space-x-2">
                                <a href="{{ route('games.edit', $game) }}" class="text-gray-600 bg-green-500 hover:bg-green-700 font-semibold py-2 px-4 rounded text-white uppercase text-xs">
                                    Edit
                                </a>
                                <form action="{{ route('games.destroy', $game) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this game?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-red-500 hover:bg-red-700 text-gray-600 font-semibold py-2 px-4 rounded text-white uppercase text-xs">
                                        Delete
                                    </button>
                                </form>
                            </div>
